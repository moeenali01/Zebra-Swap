...
contract ZebraSwap is Ownable , Pausable{
    ...
    event Swapped(address indexed user, address inputToken, address outputToken, uint256 inputAmount, uint256 outputAmount);
    ...
    function getTokenInfo(string memory symbol) external view returns (address tokenAddress, uint256 price, bool isAllowed) {
 ...
 // Get current NFT information from ZebraLiquidityNFT
    ...
    ) = liquidityNFT.getNFTInfo(nftId); // Get the current amount from the NFT
    ...
    uint256 outputAmount = (swapAmount * outputToken.price * (10 ** outputDecimals)) / (inputToken.price * (10 ** inputDecimals));
    ...
    function TokenToToken(string memory inputSymbol, string memory outputSymbol, uint256 inputAmount) external whenNotPaused {
    TokenInfo memory inputToken = allowedTokens[inputSymbol];
    TokenInfo memory outputToken = allowedTokens[outputSymbol];
    ...
    uint256 outputAmount = (swapAmount * inputToken.price * (10 ** outputDecimals)) / (outputToken.price * (10 ** inputDecimals));
